...
const generateReport = asyncHandler(async (req, res) => {
  try {
    ...
    const statsQuery = {
      ...
      query: alertBaseQuery,
      sort: [{ '@timestamp': { order: 'desc' } }],
      _source: ['rule.level', 'rule.description', 'rule.id', 'rule.groups', '@timestamp', 'agent.name', 'agent.id', 'data.srcip'],
      aggs: {
        severity_ranges: {
          range: {
            field: 'rule.level',
    ...
    const totalAlertCount = alertsResponse.data.hits?.total?.value || 0;
    const aggs = alertsResponse.data.aggregations || {};

    // Build severity counts from aggregations
    const severityBuckets = aggs.severity_ranges?.buckets || [];
    const severityCounts = { critical: 0, major: 0, minor: 0, total: totalAlertCount };
    severityBuckets.forEach(b => {
      if (b.key === 'critical') severityCounts.critical = b.doc_count;
      else if (b.key === 'major') severityCounts.major = b.doc_count;
      else if (b.key === 'minor') severityCounts.minor = b.doc_count;
    });

    // Build daily trend from aggregations
    const dailyCounts = {};
    (aggs.daily_trend?.buckets || []).forEach(b => {
      if (b.doc_count > 0) dailyCounts[b.key_as_string] = b.doc_count;
    });

    // Build top alerts from aggregations
    const topAlerts = (aggs.top_alerts?.buckets || []).map(b => {
      const maxLevel = b.max_level?.value || 8;
      return {
        description: b.key,
        severity: getSeverityLevel(maxLevel),
        count: b.doc_count,
        host: b.sample_host?.buckets?.[0]?.key || 'Unknown',
        hosts_affected: b.unique_hosts?.value || 1,
        last_seen: b.last_seen?.value_as_string
          ? formatDateTime(new Date(b.last_seen.value_as_string))
          : 'N/A'
      };
    });

    // Build alert types from aggregations
    const alertTypes = (aggs.alert_types?.buckets || []).map(b => ({
      type: b.key,
      count: b.doc_count
    }));

    // Map top raw hits as sample alerts (for any per-alert detail used in template)
    const alerts = (alertsResponse.data.hits?.hits || []).map(hit => {
    ...
    console.log(`✅ Fetched alert stats via aggregations — total: ${totalAlertCount}, critical: ${severityCounts.critical}, major: ${severityCounts.major}, minor: ${severityCounts.minor}`);
    if (alerts.length > 0) {
    ...
    // All statistics (severityCounts, topAlerts, alertTypes, dailyCounts) are already
    // computed from OpenSearch aggregations above — no per-hit loop needed.
