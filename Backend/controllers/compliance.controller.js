...
const getCompliance = asyncHandler(async (req, res) => {
  try {
    ...
    try {
      ...
      if (cachedData) {
        console.log('✅ [COMPLIANCE] Cache HIT - Data fetched from Redis (15 min cache)');
      ...
      console.log('❌ [COMPLIANCE] Cache MISS - Fetching from Wazuh API...');
      console.log('   Cache Key:', cacheKey);
      res.setHeader('X-Cache', 'MISS');
    } catch (cacheError) {
      console.warn('⚠️ [COMPLIANCE] Redis cache check failed, continuing without cache');
    }
    ...
    try {
      ...
      console.log('💾 [COMPLIANCE] Data cached in Redis for 15 minutes');
      console.log('   Total rules cached:', filtered.length);
    } catch (cacheError) {
      console.warn('⚠️ [COMPLIANCE] Redis cache set failed, continuing without cache');
    }
...
const getComplianceFramework = asyncHandler(async (req, res) => {
  try {
    ...
    try {
      const cachedData = await redisClient.get(cacheKey);
      if (cachedData) {
        console.log(`✅ [COMPLIANCE FRAMEWORK ${framework.toUpperCase()}] Cache HIT - Data fetched from Redis (15 min cache)`);
        console.log('   Cache Key:', cacheKey);
        res.setHeader('X-Cache', 'HIT');
        return res.status(200).json(
      ...
      console.log(`❌ [COMPLIANCE FRAMEWORK ${framework.toUpperCase()}] Cache MISS - Fetching from APIs...`);
      console.log('   Cache Key:', cacheKey);
      res.setHeader('X-Cache', 'MISS');
    } catch (cacheError) {
      console.warn(`⚠️ [COMPLIANCE FRAMEWORK ${framework.toUpperCase()}] Redis cache check failed, continuing without cache`);
    }
    ...
    try {
      ...
      console.log(`💾 [COMPLIANCE FRAMEWORK ${framework.toUpperCase()}] Data cached in Redis for 15 minutes`);
      console.log('   Total requirements cached:', requirements.length);
    } catch (cacheError) {
      console.warn(`⚠️ [COMPLIANCE FRAMEWORK ${framework.toUpperCase()}] Redis cache set failed, continuing without cache`);
    }
...
const handleISO27001Compliance = asyncHandler(async (req, res) => {
  try {
    ...
    try {
      const cachedData = await redisClient.get(cacheKey);
      if (cachedData) {
        console.log('✅ [COMPLIANCE ISO27001] Cache HIT - Data fetched from Redis (15 min cache)');
        console.log('   Cache Key:', cacheKey);
        res.setHeader('X-Cache', 'HIT');
        return res.status(200).json(
      ...
      console.log('❌ [COMPLIANCE ISO27001] Cache MISS - Fetching from APIs...');
      console.log('   Cache Key:', cacheKey);
      res.setHeader('X-Cache', 'MISS');
    } catch (cacheError) {
      console.warn('⚠️ [COMPLIANCE ISO27001] Redis cache check failed, continuing without cache');
    }
    ...
    try {
      ...
      console.log('💾 [COMPLIANCE ISO27001] Data cached in Redis for 15 minutes');
      console.log('   Total requirements cached:', requirements.length);
    } catch (cacheError) {
      console.warn('⚠️ [COMPLIANCE ISO27001] Redis cache set failed, continuing without cache');
    }
